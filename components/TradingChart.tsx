...
export const TradingChart: React.FC<TradingChartProps> = ({ isDark = true, onScan }) => {
    ...
    useEffect(() => {
        ...
        const initWidget = () => {
            if (mounted && window.TradingView && containerRef.current && document.getElementById(containerId.current)) {
        ...
        if (window.TradingView) {
            ...
            const scriptId = 'tv-widget-lib-global';
            if (!document.getElementById(scriptId)) {
                const script = document.createElement('script');
                script.id = scriptId;
                script.src = 'https://s3.tradingview.com/tv.js';
                script.async = true;
                script.onload = () => { if (window.TradingView) initWidget(); };
